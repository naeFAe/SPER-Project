"""
Implementation of the optimal path formulas given in the following paper:

OPTIMAL PATHS FOR A CAR THAT GOES BOTH FORWARDS AND BACKWARDS
J. A. REEDS AND L. A. SHEPP

notes: there are some typos in the formulas given in the paper;
some formulas have been adapted (cf http://msl.cs.uiuc.edu/~lavalle/cs326a/rs.c)

Each of the 12 functions (each representing 4 of the 48 possible words)
have 3 arguments x, y and phi, the goal position and angle (in degrees) of the
object given it starts at position (0, 0) and angle 0, and returns the
corresponding path (if it exists) as a list of PathElements (or an empty list).

(actually there are less than 48 possible words but this code is not optimized)
"""

from utils import *
import math
from enum import Enum
from dataclasses import dataclass, replace


class Steering(Enum):
    LEFT = -1
    RIGHT = 1
    STRAIGHT = 0


class Gear(Enum):
    FORWARD = 1
    BACKWARD = -1


@dataclass(eq=True)
class PathElement:
    param: float
    steering: Steering
    gear: Gear

    @classmethod
    def create(cls, param: float, steering: Steering, gear: Gear):
        if param >= 0:
            return cls(param, steering, gear)
        else:
            return cls(-param, steering, gear).reverse_gear()

    def __repr__(self):
        s = "{ Steering: " + self.steering.name + "\tGear: " + self.gear.name \
            + "\tdistance: " + str(round(self.param, 2)) + " }"
        return s

    def reverse_steering(self):
        steering = Steering(-self.steering.value)
        return replace(self, steering=steering)

    def reverse_gear(self):
        gear = Gear(-self.gear.value)
        return replace(self, gear=gear)


def path_length(path):
    """
    this one's obvious
    """
    return sum([e.param for e in path])


def get_optimal_path(start, end, turn_radius):
    """
    Return the shortest path from start to end among those that exist
    """
    paths = get_all_paths(start, end, turn_radius)
    return min(paths, key=path_length)

def get_all_paths(start, end, turn_radius):
    """
    Return a list of all the paths from start to end generated by the
    12 functions and their variants
    """
    path_fns = [path1, path2, path3, path4, path5, path6, \
                path7, path8, path9, path10, path11, path12]
    paths = []

    # get coordinates of end in the set of axis where start is (0,0,0)
    x, y, theta = change_of_basis(start, end)

    # Scale coordinates by turn_radius
    x /= turn_radius
    y /= turn_radius

    for get_path in path_fns:
        # get the four variants for each path type, cf article
        paths.append(scale_path(get_path(x, y, theta), turn_radius))
        paths.append(scale_path(timeflip(get_path(-x, y, -theta)), turn_radius))
        paths.append(scale_path(reflect(get_path(x, -y, -theta)), turn_radius))
        paths.append(scale_path(reflect(timeflip(get_path(-x, -y, theta))), turn_radius))

    # remove path elements that have parameter 0
    for i in range(len(paths)):
        paths[i] = list(filter(lambda e: e.param != 0, paths[i]))

    # remove empty paths
    paths = list(filter(None, paths))

    return paths

def scale_path(path, turn_radius):
    """
    Scale the parameters of each path element by the turn_radius.
    """
    return [PathElement(e.param * turn_radius, e.steering, e.gear) for e in path]


def timeflip(path):
    """
    timeflip transform described around the end of the article
    """
    new_path = [e.reverse_gear() for e in path]
    return new_path


def reflect(path):
    """
    reflect transform described around the end of the article
    """
    new_path = [e.reverse_steering() for e in path]
    return new_path


def path1(x, y, phi, turn_radius):
    """
    Formula 8.1: CSC (same turns)
    """
    phi = deg2rad(phi)
    path = []

    u, t = R(x - turn_radius * math.sin(phi), y - turn_radius + turn_radius * math.cos(phi))
    v = M(phi - t)

    path.append(PathElement.create(t, Steering.LEFT, Gear.FORWARD))
    path.append(PathElement.create(u, Steering.STRAIGHT, Gear.FORWARD))
    path.append(PathElement.create(v, Steering.LEFT, Gear.FORWARD))

    return path

def path2(x, y, phi, turn_radius):
    """
    Formula 8.2: CSC (opposite turns)
    """
    phi = M(deg2rad(phi))
    path = []

    rho, t1 = R(x + turn_radius * math.sin(phi), y - turn_radius - turn_radius * math.cos(phi))

    if rho * rho >= 4 * turn_radius * turn_radius:
        u = math.sqrt(rho * rho - 4 * turn_radius * turn_radius)
        t = M(t1 + math.atan2(2 * turn_radius, u))
        v = M(t - phi)

        path.append(PathElement.create(t, Steering.LEFT, Gear.FORWARD))
        path.append(PathElement.create(u, Steering.STRAIGHT, Gear.FORWARD))
        path.append(PathElement.create(v, Steering.RIGHT, Gear.FORWARD))

    return path

def path3(x, y, phi, turn_radius):
    """
    Formula 8.3: C|C|C
    """
    phi = deg2rad(phi)
    path = []

    xi = x - turn_radius * math.sin(phi)
    eta = y - turn_radius + turn_radius * math.cos(phi)
    rho, theta = R(xi, eta)

    if rho <= 4 * turn_radius:
        A = math.acos(rho / (4 * turn_radius))
        t = M(theta + math.pi / 2 + A)
        u = M(math.pi - 2 * A)
        v = M(phi - t - u)

        path.append(PathElement.create(t, Steering.LEFT, Gear.FORWARD))
        path.append(PathElement.create(u, Steering.RIGHT, Gear.BACKWARD))
        path.append(PathElement.create(v, Steering.LEFT, Gear.FORWARD))

    return path

def path4(x, y, phi, turn_radius):
    """
    Formula 8.4 (1): C|CC
    """
    phi = deg2rad(phi)
    path = []

    xi = x - turn_radius * math.sin(phi)
    eta = y - turn_radius + turn_radius * math.cos(phi)
    rho, theta = R(xi, eta)

    if rho <= 4 * turn_radius:
        A = math.acos(rho / (4 * turn_radius))
        t = M(theta + math.pi / 2 + A)
        u = M(math.pi - 2 * A)
        v = M(t + u - phi)

        path.append(PathElement.create(t, Steering.LEFT, Gear.FORWARD))
        path.append(PathElement.create(u, Steering.RIGHT, Gear.BACKWARD))
        path.append(PathElement.create(v, Steering.LEFT, Gear.BACKWARD))

    return path

def path5(x, y, phi, turn_radius):
    """
    Formula 8.4 (2): CC|C
    """
    phi = deg2rad(phi)
    path = []

    xi = x - turn_radius * math.sin(phi)
    eta = y - turn_radius + turn_radius * math.cos(phi)
    rho, theta = R(xi, eta)

    if rho <= 4 * turn_radius:
        u = math.acos(1 - rho * rho / (8 * turn_radius * turn_radius))
        A = math.asin(2 * math.sin(u) / rho)
        t = M(theta + math.pi / 2 - A)
        v = M(t - u - phi)

        path.append(PathElement.create(t, Steering.LEFT, Gear.FORWARD))
        path.append(PathElement.create(u, Steering.RIGHT, Gear.FORWARD))
        path.append(PathElement.create(v, Steering.LEFT, Gear.BACKWARD))

    return path

def path6(x, y, phi, turn_radius):
    """
    Formula 8.7: CCu|CuC
    """
    phi = deg2rad(phi)
    path = []

    xi = x + turn_radius * math.sin(phi)
    eta = y - turn_radius - turn_radius * math.cos(phi)
    rho, theta = R(xi, eta)

    if rho <= 4 * turn_radius:
        if rho <= 2 * turn_radius:
            A = math.acos((rho + 2 * turn_radius) / (4 * turn_radius))
            t = M(theta + math.pi / 2 + A)
            u = M(A)
            v = M(phi - t + 2 * u)
        else:
            A = math.acos((rho - 2 * turn_radius) / (4 * turn_radius))
            t = M(theta + math.pi / 2 - A)
            u = M(math.pi - A)
            v = M(phi - t + 2 * u)

        path.append(PathElement.create(t, Steering.LEFT, Gear.FORWARD))
        path.append(PathElement.create(u, Steering.RIGHT, Gear.FORWARD))
        path.append(PathElement.create(u, Steering.LEFT, Gear.BACKWARD))
        path.append(PathElement.create(v, Steering.RIGHT, Gear.BACKWARD))

    return path

def path7(x, y, phi, turn_radius):
    """
    Formula 8.8: C|CuCu|C
    """
    phi = deg2rad(phi)
    path = []

    xi = x + turn_radius * math.sin(phi)
    eta = y - turn_radius - turn_radius * math.cos(phi)
    rho, theta = R(xi, eta)
    u1 = (20 * turn_radius * turn_radius - rho * rho) / (16 * turn_radius * turn_radius)

    if rho <= 6 * turn_radius and 0 <= u1 <= 1:
        u = math.acos(u1)
        A = math.asin(2 * math.sin(u) / rho)
        t = M(theta + math.pi / 2 + A)
        v = M(t - phi)

        path.append(PathElement.create(t, Steering.LEFT, Gear.FORWARD))
        path.append(PathElement.create(u, Steering.RIGHT, Gear.BACKWARD))
        path.append(PathElement.create(u, Steering.LEFT, Gear.BACKWARD))
        path.append(PathElement.create(v, Steering.RIGHT, Gear.FORWARD))

    return path

def path8(x, y, phi, turn_radius):
    """
    Formula 8.9 (1): C|C[pi/2]SC
    """
    phi = deg2rad(phi)
    path = []

    xi = x - turn_radius * math.sin(phi)
    eta = y - turn_radius + turn_radius * math.cos(phi)
    rho, theta = R(xi, eta)

    if rho >= 2 * turn_radius:
        u = math.sqrt(rho * rho - 4 * turn_radius * turn_radius) - 2 * turn_radius
        A = math.atan2(2 * turn_radius, u + 2 * turn_radius)
        t = M(theta + math.pi / 2 + A)
        v = M(t - phi + math.pi / 2)

        path.append(PathElement.create(t, Steering.LEFT, Gear.FORWARD))
        path.append(PathElement.create(math.pi / 2, Steering.RIGHT, Gear.BACKWARD))
        path.append(PathElement.create(u, Steering.STRAIGHT, Gear.BACKWARD))
        path.append(PathElement.create(v, Steering.LEFT, Gear.BACKWARD))

    return path

def path9(x, y, phi, turn_radius):
    """
    Formula 8.9 (2): CSC[pi/2]|C
    """
    phi = deg2rad(phi)
    path = []

    xi = x - turn_radius * math.sin(phi)
    eta = y - turn_radius + turn_radius * math.cos(phi)
    rho, theta = R(xi, eta)

    if rho >= 2 * turn_radius:
        u = math.sqrt(rho * rho - 4 * turn_radius * turn_radius) - 2 * turn_radius
        A = math.atan2(u + 2 * turn_radius, 2 * turn_radius)
        t = M(theta + math.pi / 2 - A)
        v = M(t - phi - math.pi / 2)

        path.append(PathElement.create(t, Steering.LEFT, Gear.FORWARD))
        path.append(PathElement.create(u, Steering.STRAIGHT, Gear.FORWARD))
        path.append(PathElement.create(math.pi / 2, Steering.RIGHT, Gear.FORWARD))
        path.append(PathElement.create(v, Steering.LEFT, Gear.BACKWARD))

    return path

def path10(x, y, phi, turn_radius):
    """
    Formula 8.10 (1): C|C[pi/2]SC
    """
    phi = deg2rad(phi)
    path = []

    xi = x + turn_radius * math.sin(phi)
    eta = y - turn_radius - turn_radius * math.cos(phi)
    rho, theta = R(xi, eta)

    if rho >= 2 * turn_radius:
        t = M(theta + math.pi / 2)
        u = rho - 2 * turn_radius
        v = M(phi - t - math.pi / 2)

        path.append(PathElement.create(t, Steering.LEFT, Gear.FORWARD))
        path.append(PathElement.create(math.pi / 2, Steering.RIGHT, Gear.BACKWARD))
        path.append(PathElement.create(u, Steering.STRAIGHT, Gear.BACKWARD))
        path.append(PathElement.create(v, Steering.RIGHT, Gear.BACKWARD))

    return path

def path11(x, y, phi, turn_radius):
    """
    Formula 8.10 (2): CSC[pi/2]|C
    """
    phi = deg2rad(phi)
    path = []

    xi = x + turn_radius * math.sin(phi)
    eta = y - turn_radius - turn_radius * math.cos(phi)
    rho, theta = R(xi, eta)

    if rho >= 2 * turn_radius:
        t = M(theta)
        u = rho - 2 * turn_radius
        v = M(phi - t - math.pi / 2)

        path.append(PathElement.create(t, Steering.LEFT, Gear.FORWARD))
        path.append(PathElement.create(u, Steering.STRAIGHT, Gear.FORWARD))
        path.append(PathElement.create(math.pi / 2, Steering.LEFT, Gear.FORWARD))
        path.append(PathElement.create(v, Steering.RIGHT, Gear.BACKWARD))

    return path

def path12(x, y, phi, turn_radius):
    """
    Formula 8.11: C|C[pi/2]SC[pi/2]|C
    """
    phi = deg2rad(phi)
    path = []

    xi = x + turn_radius * math.sin(phi)
    eta = y - turn_radius - turn_radius * math.cos(phi)
    rho, theta = R(xi, eta)

    if rho >= 4 * turn_radius:
        u = math.sqrt(rho * rho - 4 * turn_radius * turn_radius) - 4 * turn_radius
        A = math.atan2(2 * turn_radius, u + 4 * turn_radius)
        t = M(theta + math.pi / 2 + A)
        v = M(t - phi)

        path.append(PathElement.create(t, Steering.LEFT, Gear.FORWARD))
        path.append(PathElement.create(math.pi / 2, Steering.RIGHT, Gear.BACKWARD))
        path.append(PathElement.create(u, Steering.STRAIGHT, Gear.BACKWARD))
        path.append(PathElement.create(math.pi / 2, Steering.LEFT, Gear.BACKWARD))
        path.append(PathElement.create(v, Steering.RIGHT, Gear.FORWARD))

    return path
